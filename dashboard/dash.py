...
class load_to_dashboard():
    def filter_dataframe(self, filter, df: pd.DataFrame):
        """
    ...
    def read_institution_data(self, conn):
    ...
    def read_program_data(self, conn):
    ...
    def preprocessing(self, conn):
        program_df = self.read_program_data(conn=conn)
        institutions_df = self.read_institution_data(conn=conn)
        ...
        ## Filtered List
        dte_df = dte_df.rename(columns={'INSTNM_x': 'INSTNM'})
        dte_df = dte_df[['INSTNM', 'CIPDESC', 'CREDDESC', 'CITY', 'STABBR', 'DTE_RATIO', 'ZIP']]

        return plist50_df, top_cs_institutions, top_10_cities_w_top_programs, dte_df
...
def main():
    ...
    st.dataframe(read_data.filter_dataframe(filter="Add Filter", df=top_10_cities_w_top_programs))
    st.write("## Filtered dataframe")
    st.dataframe(read_data.filter_dataframe(filter="Add Filters", df=dte_df))
